...
if ( !class_exists( "makeTicket" ) ) {
	...
	class makeTicket
	{
		...
		* Set infos du ticket
		*/
		function setInfos($name, $numInscription, $nameEvent, $dateEvent, $horaireEvent, $adress1Event, $adress2Event, $adress3Event, $adress4Event, $organisationEvent, $typePublic, $noEtudiant, $inscriptionVision )
		{
			$this->NAME = $name;
			...
			$this->ADRESS1EVENT = $adress1Event;
			$this->ADRESS2EVENT = $adress2Event;
			$this->ADRESS3EVENT = $adress3Event;
			$this->ADRESS4EVENT = $adress4Event;
			$this->ORGANISATIONEVENT = $organisationEvent;
			$this->TYPEPUBLICEVENT = $typePublic;
			$this->NUMETUDIANT = $noEtudiant;
			$this->INSCRIPTIONVISION = $inscriptionVision;
		}
...
//	                $encodedString .= "32"; //Stop
			...
			*/
			...
			$color[2] = ImageColorAllocate($this->IH, 130,130,130);
			$color[3] = ImageColorAllocate($this->IH, 100,100,100);
			$color[4] = ImageColorAllocate($this->IH, 0xcd, 0x09, 0x2f);

			imagefilledrectangle($this->IH, 0, 0, $this->WIDTH, $this->HEIGHT, $color[0]);
			
			// filigrame sciencespo
			imagefilledrectangle($this->IH, 0, 0, $this->WIDTH, $this->HEIGHT, $color[4]);
			$src = imagecreatefromgif('img/back.gif');
			imagecopy($this->IH, $src, 0, 0, 0, 0, 700, 400);
			...
			// Infos
			$ifw = imagefontwidth(1) * 20;

			// image sciencespo
			imagefilledrectangle($this->IH, 0, 0, $this->WIDTH, 64, $color[4]);
			$src = imagecreatefromgif('img/logo_scpo.gif');
			imagecopy($this->IH, $src, 0, 0, 0, 0, 390, 64);

			$positionY_infos = 90;

			// Nom
			imagestring($this->IH, 2, 10, $positionY_infos, "Nom / Prénom : ", $color[2]);
			imagestring($this->IH, 5, 150, $positionY_infos-2, $this->NAME, $color[1]);
			$positionY_infos += 20;

			// N° du ticket
			imagestring($this->IH, 2, 10, $positionY_infos, "N° d'inscription : ", $color[2]);
			imagestring($this->IH, 3, 150, $positionY_infos, $this->NUMINSCRIPTION, $color[3]);
			$positionY_infos += 50;

			// evenement
			imagestring($this->IH, 2, 10, $positionY_infos, "Evénement : ", $color[2]);
			imagestring($this->IH, 3, 150, $positionY_infos, $this->NAMEEVENT, $color[3]);
			$positionY_infos += 30;
			
			if ( $this->INSCRIPTIONVISION == 1) {
				$positionY_infos -= 10;	
				imagestring($this->IH, 3, 150, $positionY_infos, "Evénement en Visio conférence", $color[3]);
				$positionY_infos += 30;				
			}

			// date
			imagestring($this->IH, 2, 10, $positionY_infos, "Date : ", $color[2]);
			imagestring($this->IH, 3, 150, $positionY_infos, $this->DATEEVENT, $color[3]);
			$positionY_infos += 20;

			// horaire
			imagestring($this->IH, 2, 10, $positionY_infos, "Horaire : ", $color[2]);
			imagestring($this->IH, 3, 150, $positionY_infos, $this->HORAIREEVENT, $color[3]);
			$positionY_infos += 20;

			if ( $this->NUMETUDIANT <> "") {
				// numero d'etudiant
				imagestring($this->IH, 2, 10, $positionY_infos, "N° étudiant : ", $color[2]);
				imagestring($this->IH, 3, 150, $positionY_infos, $this->NUMETUDIANT, $color[3]);
				$positionY_infos += 20;
			}
			// adresse
			imagestring($this->IH, 2, 10, $positionY_infos, "Adresse : ", $color[2]);
			imagestring($this->IH, 3, 150, $positionY_infos, $this->ADRESS1EVENT, $color[3]);
			imagestring($this->IH, 3, 150, $positionY_infos+20, $this->supprimerAccent( $this->ADRESS2EVENT ) , $color[3]);
			imagestring($this->IH, 3, 150, $positionY_infos+40, $this->ADRESS3EVENT, $color[3]);
			$positionY_infos += 60;

			// Organisateur
			imagestring($this->IH, 2, 10, $positionY_infos, "Organisateur : ", $color[2]);
			imagestring($this->IH, 3, 150, $positionY_infos, $this->ORGANISATIONEVENT, $color[3]);
			$positionY_infos += 20;

			// Date d'emission
			imagestring($this->IH, 2, 10, $positionY_infos, "Date d'émission : ", $color[2]);
			imagestring($this->IH, 3, 150, $positionY_infos, date("d/m/Y"), $color[3]);
			$positionY_infos += 20;

			// Valable pour 
			imagestring($this->IH, 2, 10, $positionY_infos, "Valable pour : ", $color[2]);
			imagestring($this->IH, 3, 150, $positionY_infos, "1 personne", $color[3]);
			$positionY_infos += 20;
			...
			imagesetstyle($this->IH, $style);

			// Dessine la ligne pointillée
			imageline($this->IH, 0, $positionY_infos, $this->WIDTH, $positionY_infos, IMG_COLOR_STYLED);
